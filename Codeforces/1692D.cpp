...
void ONO(){
	string s; read(s);
	int minutos; read(minutos);
	string start = s;
	vector<string> val;
	val.push_back(start);
	while (true) {
		int hour = stoi(start.substr(0, 2));
		int minu = stoi(start.substr(3, 2));
		int totalMin = minu + minutos;
		hour = (hour + totalMin / 60) % 24;
		minu = totalMin % 60;

		string new_time = "";
		if (hour < 10) new_time += '0';
		new_time += to_string(hour);
		new_time += ':';
		if (minu < 10) new_time += '0';
		new_time += to_string(minu);
