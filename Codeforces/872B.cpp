...
void solve(){
	...
	sparse_table<int> st(r);
	if(k== 1 || (k==n) || n%k==0){
		cout<<st.query(0, n-1)<<'\n';
		return;
	}
	int div = n/k;
	int large_range = k*div;
	int first_op = st.query_over(0, large_range);
	int second_op = st.query_over(large_range, n-1);
	cout<<max(first_op,second_op)<<'\n';
}
