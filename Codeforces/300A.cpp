...
void ONO(){
	...
	vector<int> nega, posi, zero;
	for(int i = 0; i < n; i++){
		if(r[i] == 0) zero.push_back(0);
		else{
			if(r[i] < 0) nega.push_back(r[i]);
			else posi.push_back(r[i]);
		}
	}
	if(posi.empty()){
		posi.push_back(nega.back());
		nega.pop_back();
		posi.push_back(nega.back());
		nega.pop_back();
	}
	...
	cout << int(nega.size()) << " ";
	for(int x : nega) cout << x << " ";
	ps();

	cout << int(posi.size()) << " ";
	for(int x : posi) cout << x << " ";
	ps();
