...
void ONO(){
	int n, m; read(n, m);
	string s; read(s);
	vector<int> idx(m); read(idx);
	string c; read(c);
	map<char, int> mp;
	for(auto x : c) mp[x]++;
	set<int> st(idx.begin(), idx.end());
	vector<int> id(st.begin(), st.end());
	int p = int(id.size());
	vector<pair<int, char>> vals;
	int start = 0;
	for(char t = 'a'; t <= 'z' and start < p; t++){
		while(start < p and mp[t]){
			vals.emplace_back(id[start], t);
			start++;
			mp[t]--;
		}
	}

	for(auto [pos, letter] : vals){
		s[pos - 1] = letter;
	}
	ps(s);
}
