...
//	dbg(init);
	...
	for (int turno = 0; turno < p; turno++) {
		...
		//dbg("curr: ", v[x][y]);
		if (x > 0 && n >= v[x-1][y] + turno && v[x-1][y] > turno) vc.insert(v[x-1][y] + turno);
		if (r > x + 1 && n >= v[x+1][y] + turno && v[x+1][y] > turno) vc.insert(v[x+1][y] + turno);
		if (y > 0 && n >= v[x][y-1] + turno && v[x][y-1] > turno) vc.insert(v[x][y-1] + turno);
		if (c > y + 1 && n >= v[x][y+1] + turno && v[x][y+1] > turno) vc.insert(v[x][y+1] + turno);
